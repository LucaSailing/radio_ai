...
from radio_ai_package.params import (IMG_SIZE, BATCH_SIZE,
...
def make_dataset(dataframe, training=False):
    ...
    # shuffle AVANT le map/batch, dimensionné sur ce dataset-ci
    if training:
...
def preprocessing(df_filtered_CNN):
    """Prépare les données. Split PAR PATIENT : toutes les images d'un même
    patient vont dans le même ensemble (évite la fuite de données)."""
    data_model = df_filtered_CNN.copy()
    data_model['fracture_visible'] = data_model['fracture_visible'].fillna(0)
    data_model = data_model[data_model['file_path'] != ""]

    # on mélange la LISTE DES PATIENTS, pas les images
    # (patient_id existe déjà comme colonne -> pas besoin de l'extraire du filestem)
    ...
    # chaque image suit son patient -> aucun overlap possible par construction
    ...
    train_ds = make_dataset(data_train, training=True)
    val_ds   = make_dataset(data_val)
    test_ds  = make_dataset(data_test)
